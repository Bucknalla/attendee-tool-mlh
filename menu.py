...
def click_diet(self):
    ...
    window.icon         = 'resources/mlh-gs.png'
    window.run()
...
def click_web(self):
    webbrowser.open_new_tab('https://mlh.io')
...
def click_about(self):
    window = rumps.Window(
        title="About",
        message="Created by Alex Bucknall 2015",
        dimensions=(0,0)
    )
...
def click_search(self):
    ...
    if response.clicked:
        rumps.alert(
            title='Search: %s' % (response.text),
            message=mlh.search_user(response.text)
        )


if __name__ == "__main__":
    app = rumps.App("MLH Organiser Tool", icon='resources/mlh-gs.png')
    app.menu = [
        'Search',
        {
            'Attendees': {
